...
def general_data_view(id_user):
    ...
    sql3 = f"SELECT * FROM hojas_evolucion_medico WHERE id_usuario = {id_user} AND fecha_registro = (SELECT MAX(fecha_registro) FROM hojas_evolucion_medico)"
    cursor.execute(sql3)
    ...
    if result:
        if result2:
            if result3:
                ...
                st.warning("No hay registros encontrados", icon="⚠️")
        else:
            st.warning("No hay registros encontrados", icon="⚠️")
    else:
        st.warning("No hay registros encontrados", icon="⚠️")
